...
class KGDatasetWiki(KGDataset):
    ...
    def __init__(self, path, name='wikikg90m', e2r_file='e2r_scores.npz', test_mode='test-dev'):
        self.name = name
        ...
        #     del self.valid_dict['t,r->h']
        print(e2r_file)
        if os.path.exists(e2r_file):
            self.e2r = scipy.sparse.load_npz(e2r_file)
        else:
            self.e2r = None     
...
def get_dataset(data_path, data_name, format_str, delimiter='\t', files=None, has_edge_importance=False,
                test_mode='test-dev', e2r_score_file=None):
    if format_str == 'built_in':
        if data_name == 'Freebase':
            ...
            dataset = KGDatasetWiki(data_path, test_mode=test_mode, e2r_file=e2r_score_file)
        else:
