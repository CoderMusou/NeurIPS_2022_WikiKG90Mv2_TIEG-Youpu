...
class EvalSampler(object):
    ...
    def __next__(self):
        g = next(self.sampler_iter)     # from train
        r = next(self.sampler_paired_relation_iter)     # from valid or test
        ...
        unique_ids = torch.unique(torch.cat(ids))   # n-hop neighbor node
        ...
        # n-hop hit
        n_hop_find = len(torch.nonzero(unique_ids == target))
        # Expand the same relation's node for n-n relation
        if True:
            expand_node_index = torch.nonzero(self.g.edata['tid'] == r).squeeze(1)
            # print(expand_node_index.size(), expand_node_index)
            expand_node_ids = self.g.find_edges(expand_node_index)[1]
            # print(expand_node_ids.size(), expand_node_ids)
            # print(unique_ids.size(), unique_ids)
            unique_ids = torch.unique(torch.cat([unique_ids, expand_node_ids]))   # n-hop neighbor node
            # print(unique_ids.size(), unique_ids)
        if len(unique_ids) > self.num_candidates:
            if self.sparse_prob:
                ...
                unique_ids = unique_ids[e2r_matrix.nonzero()[0]]    # remove relations with probability 0
        ...
        find = len(torch.nonzero(unique_ids == target))
        return unique_ids, n_hop_find, find, torch.stack((center_id[0], torch.tensor(r), torch.tensor(target))).unsqueeze(0)
...
class EvalDataset(object):
    ...
    def __init__(self, g, dataset, args):
        ...
        if args.dataset == "wikikg90M":
            ...
            self.num_valid = 0
        if args.dataset == "wikikg90M":
            self.test_dev_dict = dataset.test_dict(mode='test-dev')
            self.num_test_dev = len(self.test_dev_dict['h,r->t']['hr'])
            self.test_challenge_dict = dataset.test_dict(mode='test-challenge')
            self.num_test_challenge = len(self.test_challenge_dict['h,r->t']['hr'])
        ...
        print('|test_dev|:', self.num_test_dev)
        print('|test_challenge|:', self.num_test_challenge)
    ...
    def get_dicts(self, eval_type):
        ...
        elif eval_type == 'test_dev':
            return self.test_dev_dict
        elif eval_type == 'test_challenge':
            return self.test_challenge_dict
        else:
            raise Exception('get invalid type: ' + eval_type)
    ...
    def create_sampler(self, eval_type, batch_size, num_hops, expand_factors, mode, node_degree, num_candidates, num_workers=32, rank=0, ranks=1):
        ...
        if self.dataset == 'wikikg90M':
            ...
            if eval_type == 'valid':
                ...
                targets = np.zeros((len(edges),), dtype=int)
