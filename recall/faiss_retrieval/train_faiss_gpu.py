...
parser.add_argument('--dstore_mmap', type=str, default="RotatE_wikikg90m_shallow_d_256_g_10.06", help='memmap where keys and vals are stored')
...
parser.add_argument('--ncentroids', type=int, default=512, help='number of centroids faiss should learn')
...
if args.dstore_fp16:
    ...
    # vals = np.load(args.dstore_mmap + '/entity_embedding.npy').astype(np.float16)
    # vals = np.load(args.dstore_mmap + '/wikikg90m_TransE_entity_embedding.npy').astype(np.float16)
    vals = np.load(args.dstore_mmap + '/entity_feat.npy').astype(np.float16)
    # keys = np.memmap(args.dstore_mmap + '/keys.npy', dtype=np.float16, mode='r',
    #                      shape=(args.dstore_size, args.dimension))
    # vals = np.load(args.dstore_mmap + '/wikikg90m_RotatE_entity.npy', dtype=np.float16, mode='r', shape=(args.dstore_size, args.dimension))
else:
    # vals = np.load(args.dstore_mmap + '/entity_embedding.npy').astype(np.float32)
    # vals = np.load(args.dstore_mmap + '/wikikg90m_TransE_entity_embedding.npy').astype(np.float32)
    vals = np.load(args.dstore_mmap + '/entity_feat.npy').astype(np.float32)
    # keys = np.memmap(args.dstore_mmap + '/keys.npy', dtype=np.float32, mode='r',
    #                  shape=(args.dstore_size, args.dimension))
    # vals = np.memmap(args.dstore_mmap + '/wikikg90m_RotatE_entity.npy', dtype=np.float32, mode='r', shape=(args.dstore_size, args.dimension))
...
if not os.path.exists(args.faiss_index + ".trained"):
    ...
    # print("gpu number ", ngpus)
    co = faiss.GpuClonerOptions()
    co.useFloat16 = True
    print("faiss gpu clone option")
    # gpu_index = faiss.index_cpu_to_all_gpus(index)
    # gpu_index = faiss.index_cpu_to_gpu(res, 0, index, co)

    print('Training Index')
    np.random.seed(args.seed)
    random_sample = np.random.choice(np.arange(vals.shape[0]), size=[min(5000000, vals.shape[0])], replace=False)
    start = time.time()
    # Faiss does not handle adding keys in fp16 as of writing this.
    # gpu_index.train(keys[random_sample].astype(np.float32))
    print(random_sample[:10])
    # print(vals[random_sample][:10])
    vals_random_sample = vals[random_sample]
    print("finish shuffling data...")
    index.train(vals_random_sample.astype(np.float32))
