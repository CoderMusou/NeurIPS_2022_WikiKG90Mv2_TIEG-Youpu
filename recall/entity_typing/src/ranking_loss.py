...
class RankingLoss(nn.Module):
    ...
    def __init__(self, m: float, gamma: float, weight=None) -> None:
    ...
    def forward(self, y_pred, y_true):
        ...
        if self.weight:
            loss = self.soft_plus(torch.log(torch.sum(self.weight * torch.exp(logit_n), -1)) +
                                  torch.log(torch.sum(self.weight * torch.exp(logit_p), -1)))
        else:
