...
class PathCon(nn.Module):
    def __init__(self, args, n_relations, weight=None):
        super(PathCon, self).__init__()
        self._parse_args(args, n_relations, weight)
        self._build_model()

    def _parse_args(self, args, n_relations, weight):
        ...
        if self.use_bce:
            ...
            self.loss = RankingLoss(args.margin, args.gamma, weight)
        else:
    ...
    def train_step(self, optimizer, batch, uni_weight):
        self.train()

        start1 = time.time()
        optimizer.zero_grad()
        logits_tail = self(batch)

        if self.use_bce or self.use_ranking_loss:
            labels = batch['label_sets']
            loss = self.loss(logits_tail, labels)
        else:
            loss = self.loss(logits_tail, batch['labels'])
        ...
        return loss.item(), forward_time, backward_time, update_time

    def test_step_loss(self, batch):
        self.eval()
        with torch.no_grad():
            logits_tail = self(batch)
            if self.use_bce or self.use_ranking_loss:
    ...
    def test_step_score(self, batch):
        self.eval()
        with torch.no_grad():
            if self.use_bce and self.use_ranking_loss:
                logits_tail = F.sigmoid(self(batch))
            else:
                logits_tail = F.softmax(self(batch), dim=-1)
