    ...
    def __init__(self, args, path, name='wikikg90m'):
        ...
        if args.eval_percent > 0:
            ...
            self.dataset.valid_dict['h,r->t'][
                't_candidate'] = np.load(os.path.join(path, 
                'wikikg90m-v2/processed/val_t_candidate.npy'))[:num_valid]
            self.dataset.valid_dict['h,r->t'][
        ...
        num_test = len(self.test_dev_dict['h,r->t']['hr'])
        num_test = int(num_test * args.test_percent)
        self.test_dev_dict['h,r->t']['hr'] = self.test_dev_dict[
            'h,r->t']['hr'][:num_test]
        self.test_dev_dict['h,r->t'][
            't_candidate'] = np.load(os.path.join(
            path, 'wikikg90m-v2/processed/test_t_candidate.npy'))[:num_test]
        print("num_test", num_test)

        self.test_challenge_dict = self.dataset.test_dict('test-challenge')
        self.test_challenge_dict['h,r->t']['t_candidate'] = np.load(
                os.path.join(path, 'wikikg90m-v2', 'processed', 'test_challenge_t_candidate.npy'))
        ...
        self.test_dict = self.test_dev_dict
