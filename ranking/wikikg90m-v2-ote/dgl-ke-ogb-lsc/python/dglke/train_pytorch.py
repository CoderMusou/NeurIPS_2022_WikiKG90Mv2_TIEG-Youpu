...
def load_model_from_checkpoint(args, n_entities, n_relations, ckpt_path,
    ...
    if args.encoder_model_name != 'shallow':
...
def train(args,
    ...
    for step in range(0, args.max_step):
        ...
        # if True:
        if (step+1) % 100000 == 0 and rank == 0:
            save_model(args, model, None, None)
            logging.info('proc {} model saved'.format(rank))
        if args.valid and ((
                step + 1
        ) % args.eval_interval == 0 or step == 0) and valid_samplers is not None:
            logging.info('start evaluation...')
            valid_start = time.time()
            if args.strict_rel_part or args.soft_rel_part:
                model.writeback_relation(rank, rel_parts)
            logging.info('finish writing back relation')
            ...
            if args.soft_rel_part:
                model.prepare_cross_rels(cross_rels)
            logging.info('start loading validation pkl...')
            #     barrier.wait()
            if rank == 0:
                valid_result_dict = th.load(os.path.join(args.save_path, "valid_{}_{}.pkl".format(0, step)))
                ...
                logging.info("finish loading validation set...")
