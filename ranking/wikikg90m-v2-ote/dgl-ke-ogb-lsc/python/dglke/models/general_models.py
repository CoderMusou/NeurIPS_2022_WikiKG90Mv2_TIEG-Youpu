...
class KEModel(object):
    ...
    def __init__(self,
        ...
        self.create_hr_embedding = self.score_func.create_hr_embedding()
    ...
    def forward_test_wikikg(self, query, ans, candidate, mode, gpu_id=-1):
        ...
        temp = []
        for idx in range(argsort.size(0)):
            temp.append(candidate[idx][argsort[:, :10][idx]])
        pred_t = th.stack(temp, dim=0)
        return pred_t, scores
    ...
    def forward_entity_embedding(self, candidate, mode, gpu_id=-1):
        """Do the forward and generate ranking results.
        ...
        gpu_id : int
            Which gpu to accelerate the calculation. if -1 is provided, cpu is used.
        """
        scores = self.predict_entity_embedding(
             candidate, mode, to_device=cuda, gpu_id=gpu_id, trace=False)
        return scores

    def predict_entity_embedding(self,
                             candidate,
                             mode,
                             to_device=None,
                             gpu_id=-1,
    ...
    def forward_validate_hr_wikikg(self, query, ans, mode, gpu_id=-1):
        """Do the forward and generate ranking results.
        ...
        gpu_id : int
            Which gpu to accelerate the calculation. if -1 is provided, cpu is used.
        """
        scores = self.generate_hr_embedding_faiss(
            query, mode, to_device=cuda, gpu_id=gpu_id, trace=False)
        # argsort = F.argsort(scores, dim=1, descending=True)
        # return argsort[:, :1000]
        return scores

    def generate_hr_embedding_faiss(self, query, mode, to_device=None, gpu_id=-1, trace=False):
        num_chunks = len(query)
        ...
        if mode == 'h,r->t':
            ...
            hr_embedding = self.create_hr_embedding(head, rel, chunk_size).squeeze(1)
