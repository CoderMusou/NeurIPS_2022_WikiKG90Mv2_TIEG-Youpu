...
class OTEScore(nn.Module):
    ...
    def create_hr_embedding(self):
        def fn(heads, relations, chunk_size):
            relations = self.orth_rel_embedding(relations)
            inputs_size = heads.size()
            assert inputs_size[:-1] == relations.size()[:-1]
            num_dim = heads.size(-1)
            inputs = heads.view(-1, 1, self.num_elem)
            if self.use_scale:
                rel = relations.view(-1, self.num_elem, self.num_elem + 1)
                scale = self.get_scale(rel[:, :, self.num_elem:])
                scale = scale / scale.norm(dim=-1, keepdim=True)
                rel_scale = rel[:, :, :self.num_elem] * scale
                outputs = torch.bmm(inputs, rel_scale)
            else:
                rel = relations.view(-1, self.num_elem, self.num_elem)
                outputs = torch.bmm(inputs, rel)
            outputs = outputs.view(chunk_size, 1, inputs_size[-1])
            return outputs
