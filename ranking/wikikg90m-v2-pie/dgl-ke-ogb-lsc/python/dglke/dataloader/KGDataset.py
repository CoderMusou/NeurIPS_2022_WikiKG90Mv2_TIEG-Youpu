...
class KGDatasetWiki(KGDataset):
    ...
    def __init__(self, path, name='wikikg90m', test_mode='test-all'):
        self.name = name
        ...
        self.valid_dict['h,r->t']['t_candidate'] = np.load(os.path.join(path, 'wikikg90m-v2-pie', 'processed', 'val_t_candidate.npy'))
        self.test = None
        self.test_dev_dict = self.dataset.test_dict('test-dev')
        self.test_challenge_dict = self.dataset.test_dict('test-challenge')
        if test_mode == 'test-all':
            self.test_dev_dict['h,r->t']['t_candidate'] = np.load(os.path.join(path, 'wikikg90m-v2-pie', 'processed', 'test_t_candidate.npy'))
            self.test_challenge_dict['h,r->t']['t_candidate'] = np.load(os.path.join(path, 'wikikg90m-v2-pie', 'processed', 'test_challenge_t_candidate.npy'))
        elif test_mode == 'test-challenge':
            self.test_challenge_dict['h,r->t']['t_candidate'] = np.load(os.path.join(path, 'wikikg90m-v2-pie', 'processed', 'test_challenge_t_candidate.npy'))
        elif test_mode == 'test-dev':
            self.test_dev_dict['h,r->t']['t_candidate'] = np.load(os.path.join(path, 'wikikg90m-v2-pie', 'processed', 'test_t_candidate.npy'))
        else:
            raise NotImplementedError("The {} test mode not exists! Please choice test-challenge, test-dev or test-all!"
                                      .format(test_mode))
        ...
        if 't,r->h' in self.test_dev_dict:
            del self.valid_dict['t,r->h']
...
def get_dataset(data_path, data_name, format_str, delimiter='\t', files=None, has_edge_importance=False, test_mode='test-all'):
