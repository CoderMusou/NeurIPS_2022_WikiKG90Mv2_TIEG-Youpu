...
class ArgParser(CommonArgParser):
    def __init__(self):
        ...
        self.add_argument('--only_valid', action='store_true')
        self.add_argument('--checkpoint', default='', type=str)
        self.add_argument('--valid_dict', default='', type=str)
        self.add_argument('--test_challenge_dict', default='', type=str)
        self.add_argument('--test_dev_dict', default='', type=str)
        self.add_argument('--test_dev', action='store_true',
                          help='Evaluate the model on the test-dev set after the model is trained.')
        self.add_argument('--test_challenge', action='store_true',
                          help='Evaluate the model on the test-challenge set after the model is trained.')
        ...
        self.add_argument('--seed', type=int, default=0,
                          help='random seed')
        self.add_argument('--test_mode', default='test-all', type=str,
                          help='Evaluate the model on the validation set in the training.')
...
def main():
    ...
    if args.valid_dict:
        dataset.valid_dict['h,r->t']['t_candidate'] = np.load(args.valid_dict)
    if args.test_challenge_dict:
        dataset.test_challenge_dict['h,r->t']['t_candidate'] = np.load(args.test_challenge_dict)
    if args.test_dev_dict:
        dataset.test_dev_dict['h,r->t']['t_candidate'] = np.load(args.test_dev_dict)
    ...
    if args.test_dev:
        if args.num_test_proc > 1:
            ...
            for i in range(args.num_test_proc):
                ...
                test_dev_sampler_tail = eval_dataset.create_sampler('test-dev', args.batch_size_eval,
                                                                args.neg_sample_size_eval,
                                                                args.neg_sample_size_eval,
                                                                args.eval_filter,
                                                                mode='tail',
                ...
                test_dev_sampler_tails.append(test_dev_sampler_tail)
    ...
    if args.test_challenge:
        if args.num_test_proc > 1:
            ...
            for i in range(args.num_test_proc):
                ...
                #                                                  num_workers=args.num_workers,
                #                                                  rank=i, ranks=args.num_test_proc)
                test_challenge_sampler_tail = eval_dataset.create_sampler('test-challenge', args.batch_size_eval,
                                                                args.neg_sample_size_eval,
                                                                args.neg_sample_size_eval,
                                                                args.eval_filter,
                                                                mode='tail',
            ...
            #                                                 num_workers=args.num_workers,
            #                                                 rank=0, ranks=1)
            test_challenge_sampler_tail = eval_dataset.create_sampler('test-dev', args.batch_size_eval,
                                                            args.neg_sample_size_eval,
                                                            1,
                                                            args.eval_filter,
                                                            mode='tail',
    ...
    if args.checkpoint:
        ...
        args.save_path = args.checkpoint
        model = load_model_from_checkpoint(args, dataset.n_entities, dataset.n_relations, args.checkpoint,
    ...
    if args.num_proc > 1:
        ...
        for i in range(args.num_proc):
            ...
            test_dev_sampler = [test_dev_sampler_tails[i]] if args.test_dev else None
            test_challenge_sampler = [test_challenge_sampler_tails[i]] if args.test_challenge else None
        ...
        test_dev_samplers = [test_dev_sampler_tail] if args.test_dev else None
        test_challenge_samplers = [test_challenge_sampler_tail] if args.test_challenge else None
        ...
        train(args, model, train_sampler, valid_samplers, test_dev_samplers, test_challenge_samplers, rel_parts=rel_parts)
