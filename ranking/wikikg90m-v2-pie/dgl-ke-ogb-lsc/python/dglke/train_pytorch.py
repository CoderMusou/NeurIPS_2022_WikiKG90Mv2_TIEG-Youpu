...
def load_model_from_checkpoint(args, n_entities, n_relations, ckpt_path, ent_feat_dim, rel_feat_dim):
    ...
    if args.encoder_model_name in ['concat', 'concat_v1']:
        model_path = os.path.join(ckpt_path, args.dataset + "_" + args.model_name + "_mlp")
        model_dict = torch.load(
            model_path,
            map_location=torch.device('cpu'))["transform_state_dict"]
        for key, value in model_dict.items():
            model_dict[key] = value.cpu()
        model.transform_net.load_state_dict(model_dict)
        print("load mlp parameters done.")
    return model


def train(args, model, train_sampler, valid_samplers=None, test_dev_samplers=None, test_challenge_samplers=None, rank=0, rel_parts=None, cross_rels=None, barrier=None, client=None):
    logs = []
    ...
    for step in range(0, args.max_step):
        ...
        if (args.valid and (step + 1) % args.eval_interval == 0 and step > 1 and valid_samplers is not None) \
            ...
            if valid_samplers is not None:
                ...
                # else:
                valid_input_dict = test(args, model, valid_samplers, step, rank, mode='Valid', save_scores=save_scores)
                th.save(valid_input_dict, os.path.join(
                    args.save_path, "valid_{}_{}.pkl".format(rank, step)))
            if test_dev_samplers is not None:
                test_input_dict = test(args, model, test_dev_samplers, step, rank, mode='Test', save_scores=save_scores)
                th.save(test_input_dict, os.path.join(
                    args.save_path, "test_dev_{}_{}.pkl".format(rank, step)))
            if test_challenge_samplers is not None:
                test_input_dict = test(args, model, test_challenge_samplers, step, rank, mode='Test', save_scores=save_scores)
                th.save(test_input_dict, os.path.join(
                    args.save_path, "test_challenge_{}_{}.pkl".format(rank, step)))
            ...
            if rank == 0 and not args.only_valid:
                save_model(args, model, None, None)
                print('proc {} model saved in step {}'.format(rank, step))

    print('proc {} takes {:.3f} seconds'.format(rank, time.time() - train_start))
    time.sleep(10)
    if rank == 0 and not args.no_save_emb and not args.only_valid:
        save_model(args, model, None, None)
...
def test(args, model, test_samplers, step, rank=0, mode='Test', save_scores=False):
    if len(args.gpu) > 0:
        gpu_id = args.gpu[rank %
                          len(args.gpu)] if args.mix_cpu_gpu and args.num_proc > 1 else args.gpu[0]
    else:
    ...
    with th.no_grad():
        ...
        if 'h,r->t' in answers:
            ...
            input_dict['h,r->t'] = {'t': th.cat(
                answers['h,r->t'], 0), 't_pred_top10': th.cat(logs['h,r->t'], 0)}
            if save_scores:
                input_dict['scores'] = th.cat(scores, 0)
...
@thread_wrapped_func
def train_mp(args, model, train_sampler, valid_samplers=None, test_dev_samplers=None, test_challenge_samplers=None, rank=0, rel_parts=None, cross_rels=None, barrier=None):
    if args.num_proc > 1:
        th.set_num_threads(args.num_thread)
    train(args, model, train_sampler, valid_samplers,
          test_dev_samplers, test_challenge_samplers, rank, rel_parts, cross_rels, barrier)
